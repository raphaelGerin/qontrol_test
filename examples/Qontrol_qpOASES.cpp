...
{
  model =
      Model::RobotModel<Model::RobotModelImplType::PINOCCHIO>::loadModelFromFile(urdf_path);
  auto solver =
      std::make_shared<Solver::Solver<Solver::SolverImplType::qpOASES>>();
  ...
  init_pose = pinocchio::SE3(model->getFramePose(model->getTipFrameName()).matrix());
  auto target_pose = pinocchio::SE3(init_pose.rotation(),init_pose.translation() - Eigen::Vector3d(0.1,0.0,0.1));

  traj.createTrajectory(init_pose.toHomogeneousMatrix(),target_pose.toHomogeneousMatrix());
}
...
int main(int argc, const char** argv) {
  ...
  mujoco_qontrol.urdf_path = "./resources/"+robot+"/robot.urdf";
