...
mjModel* MujocoSim::LoadModel(const char* file, mj::Simulate& sim) {
  ...
  if (mju::strlen_arr(filename)>4 &&
    ...
    mnew = mj_loadXML(filename, nullptr, load_error, mj::Simulate::kMaxFilenameLength);
...

    {
      ...
      if (m) {
        ...
        if (sim.run) {
          ...
          mju_copy(d->ctrl,m->body_gravcomp, m->nu); 
          // inject noise
          ...
          if (elapsedSim < 0 || elapsedCPU.count() < 0 || syncCPU.time_since_epoch().count() == 0 ||
            ...
            // sim.apply_pose_pertubations(0);  // move mocap bodies only
            // sim.apply_force_perturbations();
        ...
        // paused
          ...
          // sim.apply_pose_pertubations(1);  // move mocap and dynamic bodies
...
void MujocoSim::PhysicsThread(mj::Simulate* sim, std::string filename) {
...
void MujocoSim::sendJointVelocity(Eigen::VectorXd command)
{
  mju_zero(d->qvel,m->nu);
...
void MujocoSim::sendJointTorque(Eigen::VectorXd command)
{
