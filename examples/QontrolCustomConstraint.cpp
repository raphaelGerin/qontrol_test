...
public:
  ...
  void updateController() override
  {
    ...
    custom_constraint->setConstraintMatrix(Eigen::MatrixXd::Identity(
        model->getNrOfDegreesOfFreedom(), model->getNrOfDegreesOfFreedom()));
    custom_constraint->setUpperBounds(model->getJointVelocityLimits());
    custom_constraint->setLowerBounds(-model->getJointVelocityLimits());
