...
For the torque control and the velocity examples,the robot main task consists in following a simple trajectory defined in Cartesian space.
...
To achieve the objective, we need to modify the trajectory. Thus, we created a method "createTestTrajectory" that takes as argument the initial robot pose and from it, generates the desired trajectory in a new file "trajectory_test.csv".
For the trajectory generation, we used a classical trapezoidal velocity profile to ensure smooth movement.

We first start with the move along x axis : 
...
We must choose $v_{x,max}$ and $a_{x}$ st $\frac{v_{x,max}^2}{a_{x}} \leq \Delta_{x}$, with $\Delta_{x}$ the desired displacement along x axis.
...
With $v_{x,max}$ = 0.05m/s, $a_{x} = 0.1m/s²$, $v_{z,max}$ = -0.05m/s and $a_{z} = -0.1m/s²$ , the robot moves smoothly and follows the desired trajectory. An overview of the trajectory is displayed on the following figure (robot at final position).
...
First, we looked at the pycapacity documentation and more especially a [tutorial](https://auctus-team.github.io/pycapacity/examples/reachable_workspace.html) that explains how to display the non-convexe reachable workspace of a Panda robot.
...
Increasing the number of sample improves the accuracy but it increases the computation time. 
...
First, we looked at the pycapacity documentation and more especially an [example](https://auctus-team.gitlabpages.inria.fr/people/antunskuric/pycapacity/examples/pinocchio.html) that explains how to display the force capacity polytope of Panda robot.
...
The pycapacity function "force_polytope(J,t_max,t_min)" computes the polytope from the position Jacobian, the maximal and minimal torques (from the robot model). 


Note that we must add a translation to the polytope to display it at the correct place using Meshcat (with respect to the robot end effector).
...
As soon as we fold it, the polytope becomes a little bit more spherical (several actuators can get involved in the force generation).